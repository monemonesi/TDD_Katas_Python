...
def test_jumping_on_clouds_should_return_2_with_4_green_clouds() -> None:
    """Test path with 4 greens clouds"""
    assert Calculator.jumping_on_clouds([0, 0, 0, 0]) == 2


def test_jumping_on_clouds_return_the_correct_input_when_5_clouds() -> None:
    assert Calculator.jumping_on_clouds([0, 0, 1, 0, 0]) == 3


@pytest.mark.parametrize('path_result', [t for t in FIVE_CLOUDS_PATHS_RESULT])
def test_jumping_on_clouds_return_the_correct_input_when_5_clouds(path_result: ()) -> None:
    assert Calculator.jumping_on_clouds(path_result[0]) == path_result[1]
